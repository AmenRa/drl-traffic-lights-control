...
class Simulator:
    ...
    def _compute_reward(self, current_waiting_time, step):
        reward = 1
        if current_waiting_time > 2000:
            reward = 0.01
        elif current_waiting_time > 1000:
            reward = 0.1
        elif current_waiting_time > 500:
            reward = 0.3
        elif current_waiting_time > 300:
            reward = 0.5
        elif current_waiting_time > 100:
            reward = 0.9
        return reward
    ...
    def do_step(self, step):
        ...
        # return self.sumocfg, self.state_size, self.agent, self.max_steps, self.cumulative_reward, self.cumulative_waiting_time, self.throughput, self.cumulative_intersection_queue, self.yellow_phase, self.green_phase, self.yellow_phase_step_count, self.green_phase_step_count, self.action, self.previous_action, self.label, self.junction_id
